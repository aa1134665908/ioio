...
export class ChatComponent implements OnInit {
  modelGroup: ModelGroup[] = [
    {
      ...
      content:
        'ChatGPT 是基于 OpenAI 的 GPT 系列模型开发的对话系统，能够理解和生成自然语言文本，广泛应用于聊天、问答、内容创作等场景，帮助用户解决各种问题并提供信息支持。',
      subModels: [
        {
          name: 'GPT-3.5',
          imageUrl: '../../assets/icn_gpt3.5_off.png',
    ...
    },
    {
      ...
      content:
        '更多实验性的模型，感受不一样的效果，注意此模式为探索性质，稳定性较弱',
      subModels: [
        {
  ...
  getImageStyle(index: number): { filter?: string } {
    if (this.isCaptchaVisible === index || this.hoveredIndex === index) {
      if (index === 0) {
        return {
          filter:
            'brightness(0) saturate(100%) invert(71%) sepia(17%) saturate(1354%) hue-rotate(137deg) brightness(89%) contrast(90%)',
        };
      } else if (index === 1) {
        return {
          filter:
            'brightness(0) saturate(100%) invert(20%) sepia(100%) saturate(1854%) hue-rotate(247deg) brightness(115%) contrast(88%)',
        };
      }
    }
    return {}; // 不应用滤镜
  ...
  sendSelectedSubModelName(groupIndex: number) {
    ...
    if (typeof selectedModel.selectedSubModelIndex === 'number') {
      // 获取当前选择的子模型
      const selectedSubModel =
        selectedModel.subModels[selectedModel.selectedSubModelIndex];
  ...
  addItem(id: string, content: string, role: 'user' | 'assistant') {
    ...
    this.chatDataService.addItem(
      id,
      newMessage,
      this.chatDataService.getSelectedModel()
    ); // 调用服务的方法添加新项
  }
  ...
  sendMessage() {
    ...
    this.aimanagerService.sendMessage(
      messageId,
      this.chatDataService.getSelectedModel()
    );
  }
